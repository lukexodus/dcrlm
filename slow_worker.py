...
# Slow Worker: lag-test variant of worker_client.py for CP-6.
# Identical to worker_client.py except send_request sleeps 2 seconds
# before sending — simulates a worker with high network latency.
# Used to demonstrate that Lamport logical order overrides physical arrival order.
...
def send_request(sock, clock, worker_id):
    ...
    CP-6 MODIFICATION: sleeps 2 seconds before actually sending the message.
    This simulates a slow/lagged network path.

    The Lamport timestamp is stamped BEFORE the sleep, so the timestamp
    reflects when the worker DECIDED to request, not when the message arrived.
    Despite the delay, a worker with a lower timestamp still wins the queue.
    """

    # Increment Lamport clock before sending (Rule 2)
    ...
    # --- CP-6: simulate network lag ---
    print(f"[{worker_id}] Simulating network lag — sleeping 5 seconds before send.")
    print(f"[{worker_id}] *** Switch to WA and WB terminals NOW and type 'request' ***")
    time.sleep(5)
    # ------------------------------------

    # Send message (arrives late at LM due to sleep above)
    send_json(sock, msg)
...
def input_loop(sock, clock, worker_id, state):
    ...
    print(f"  request  - Request the lock (2-second lag before send)")
...
def start_worker(worker_id, ns_host, ns_port):
    """
    Main entry point for Slow Worker Client.
    Identical to worker_client.start_worker — only send_request differs.
    """
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Slow Worker Client — lag-test variant for CP-6"
    )
